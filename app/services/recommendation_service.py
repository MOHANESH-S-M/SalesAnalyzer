...
async def get_inventory_recommendations(db: AsyncSession, user_id: int):
    ...
    for p in products:
        ...
        if p.total_quantity >= avg_sales * 1.5:
            status = "High Demand - Restock Soon"
            priority = "High"
        elif p.total_quantity <= avg_sales * 0.5:
    ...
    # Sort by priority (High demand first)
    recommendations.sort(key=lambda x: x['total_sold'], reverse=True)
