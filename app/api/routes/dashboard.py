...
async def line_graph(db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
async def pie_chart(db: Session = Depends(get_db), current_user = Depends(get_current_user)):
